...
12.) Чтобы успешно подключаться по ssh к себе же ( на локалхост) нужно добавить публичный ключ -t например ssh -t host (ключ -t вызывает принудительное выделения псевдотерминала)
